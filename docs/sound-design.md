...
- **Ring / droplets / bird chirps:** inharmonic bell modes, scattered water resonances, and answering rising/falling bird phrases.
...
- **Rain / waves / thunder:** dense independent rain grains, breaking/receding surf, and a crack with successive low thunder rolls. These designs are unchanged.
...
| `envelope.attack_s`, `envelope.decay_s` | 0.001–0.5 s, 0.025–2 s |
| `envelope.sustain_level`, `envelope.release_s` | 0 (one-shot), 0–0.5 s (reserved; gate stays held) |
...
Render rates: 22050–96000 Hz. Values must be finite. Envelopes/tails are bounded to under six seconds; arbitrary graphs are not accepted through the command interface. Identical recipes/rates reproduce identical PCM on the same target; small floating-point differences may occur between native architectures and WASM.
