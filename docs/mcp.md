...
[features]
default = []
mcp = ["dep:rmcp", "dep:tokio", "dep:tokio-util", "dep:futures-util", "dep:axum", "dep:uuid"]
...
Tool definitions include read-only, destructive, idempotent, and open-world annotations. Successful tools return structured content alongside their text results; the structured result for `list_sounds` is an object with a `sounds` array. Invalid tool arguments and synthesis errors return visible tool errors. Unknown tools return a protocol error.
...
These tests launch the real server process, exercise both SDK lifecycles, inspect tool annotations and structured results, generate and mix WAV files, reproduce saved recipes, and check validation, cancellation, protocol responsiveness during rendering, bounded framing, and non-overwriting exports.

Lifecycle tests also start the background HTTP server, generate assets through MCP, verify editing state across requests, reject duplicate starts and invalid origins, stop it, and restart it.
